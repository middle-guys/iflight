module Sms
  class SmsTemplateBinding
    ...
    def self.get_content(order)
      ...
      sms_one_way = "HOLDING one way #{@ori_code}-#{@des_code}. DEPARTURE: #{@depart_from_time} #{@depart_date} - #{@depart_airline} (#{@depart_flight_code}) - booking code #{@depart_booking_code}. Passengers: #{@passengers}. Total #{@total_price.round(0)} VND. Expired at #{@time_expired}. Hotline 0933554440"
      sms_round_trip = "HOLDING round trip #{@ori_code}-#{@des_code}. DEPARTURE: #{@depart_from_time} #{@depart_date} - #{@depart_airline} (#{@depart_flight_code}) - booking code #{@depart_booking_code}. RETURN: #{@return_from_time} #{@return_date} - #{@return_airline} (#{@return_flight_code}) - booking code #{@return_booking_code}. Passengers: #{@passengers}. Total #{@total_price.round(0)} VND. Expired at #{@time_expired}. Hotline 0933554440"
